...
Когда вызывается `panic`, текущее выполнение функции `f` останавливается и функция поднимается вверх по стеку вызовов: в `main`. Поскольку паника перехватывается с помощью `recover`, в `main` она не останавливает выполнение горутины:
```bash
a
recover foo
```
...
Когда уместно вызывать панику?
В Go `panic` используется для обозначения по-настоящему исключительных ситуаций, таких как ошибка программиста. Например, если мы обратимся к пакету `net/http`, то заметим, что в методе `WriteHeader` есть вызов функции `checkWriteHeaderCode` для проверки правильности кода состояния:
```go
...
func Register(name string, driver driver.Driver) {
    ...
    if _, dup := drivers[name]; dup {
        panic("sql: Register called twice for driver " + name) // Паника, если драйвер уже зарегистрирован
    }

    drivers[name] = driver
}
```
Эта функция вызывает состояние паники, если драйвер равен `nil` (`driver.Driver` — это интерфейс) или уже зарегистрирован. Оба случая снова будут считаться ошибками программиста.

Другой случай для паники — невозможность инициализировать обязательную зависимость. Например, сервис создания учётных записей клиентов должен проверять валидность email с помощью регулярного выражения. В Go пакет `regexp` предлагает две функции: `Compile` (возвращает ошибку) и `MustCompile` (при ошибке вызывает панику). Поскольку без скомпилированного регулярного выражения сервис не сможет выполнять ключевую функцию — проверку email, — корректно использовать `MustCompile`, допуская панику при ошибке компиляции.
...
## ОШИБКА #49: ИГНОРИРОВАТЬ ОБОРАЧИВАНИЕ ОШИБКИ
...
Оборачивание — это упаковка ошибки внутри контейнера-обертки, который делает доступной и исходную ошибку. Есть два основных сценария использования оборачивания ошибок:
1. Добавление дополнительного контекста к ошибке.
Мы получаем запрос от конкретного пользователя на доступ к ресурсу базы данных, но во время запроса получаем ошибку «Отказ в доступе» («permission denied»). Если информация об этой ошибке заносится в какой-то журнал, то для целей отладки нужно добавить дополнительный контекст. В этом случае мы можем обернуть ошибку, чтобы указать, кто был пользователем и к какому ресурсу был запрос.

2. Маркировка ошибки как специфической.
Предположим, что вместо добавления контекста мы хотим пометить ошибку. Например, реализовать обработчик HTTP, который проверяет, все ли ошибки, полученные при вызове функций, относятся к типу Forbidden, чтобы можно было вернуть код состояния 403. В этом случае мы можем обернуть эту ошибку внутри Forbidden.

В обоих случаях исходная ошибка остается доступной. Следовательно, вызывающая сторона также может обработать ошибку, развернув ее и перепроверив источник ошибки. Обратите внимание, что иногда есть смысл использовать оба подхода.
...
func Foo() error {
    err := bar()
    if err != nil {
        // Как возвратить ошибку?
    }
    // ...
}
```

Вернуть эту ошибку напрямую. Если мы не хотим ее пометить и понимаем, что нет какого-либо полезного контекста, который имеет смысл добавить, то такой подход вполне подойдет:
```go
if err != nil {
    return err
}
...
```
Этот код оборачивает исходную ошибку, чтобы можно было добавлять дополнительный контекст без необходимости создания другого типа ошибки. Поскольку исходная ошибка остается доступной, клиент может развернуть родительскую ошибку, а затем проверить, относится ли исходная ошибка к какому-либо специфическому типу или значению.
...
## ОШИБКА #50: НЕТОЧНАЯ ПРОВЕРКА ТИПА ОШИБКИ

При использовании способа оборачивания ошибок `fmt.Errorf` с `%w` важно изменить и способ проверки типа ошибки на его специфичность, иначе обработка ошибок может оказаться неточной. 

Пример. Напишем обработчик HTTP для возврата суммы транзакции по идентификатору (ID). Обработчик будет анализировать запрос, чтобы получить данные об этом ID и информацию о сумме из базы данных (БД). Реализация такой функции может приводить к ошибкам в двух случаях:
...
func (t transientError) Error() string { // Создается пользовательский тип transientError
    return fmt.Sprintf("transient error: %v", t.err)
}

func getTransactionAmount(transactionID string) (float32, error) {
    if len(transactionID) != 5 {
        return 0, fmt.Errorf("id is invalid: %s", transactionID) // Возврат простой ошибки, если ID транзакции недействителен
    }

    amount, err := getTransactionAmountFromDB(transactionID)
    if err != nil {
        return 0, transientError{err: err} // Возврат ошибки типа transientError в случае сбоя при запросе в БД
    }

    return amount, nil
}
```
`getTransactionAmount` возвращает ошибку, используя `fmt.Errorf`, если ID недействителен. Но если получить данные о сумме транзакции из БД не удается, `getTransactionAmount` оборачивает ошибку в тип `transientError`.
...
func handler(w http.ResponseWriter, r *http.Request) {
    transactionID := r.URL.Query().Get("transaction") // Извлечение ID транзакции

    amount, err := getTransactionAmount(transactionID) // Вызов getTransactionAmount, в котором заключена вся логика обработки ошибки
    if err != nil {
        switch err := err.(type) { // Проверка типа ошибки; возврат кода 503, если ошибка имеет тип transientError; во всех других случаях — возврат кода 400
            case transientError:
...
func getTransactionAmount(transactionID string) (float32, error) {
    ...
    amount, err := getTransactionAmountFromDB(transactionID)
    if err != nil {
        return 0, fmt.Errorf("failed to get transaction %s: %w", transactionID, err) // Оборачивание ошибки вместо ее прямого возврата в виде transientError
    }
...
func getTransactionAmountFromDB(transactionID string) (float32, error) {
    // ...
    if err != nil {
        return 0, transientError{err: err} // Эта функция возвращает transientError
    }
    // ...
}
```

Если мы запустим этот код, он всегда вернет 400, независимо от типа ошибки, поэтому случай, когда должна быть выдана `TransientError`, никогда не появится.
Как объяснить такое поведение?
Перед рефакторингом функция `getTransactionAmount` возвращала `transientError`. После же него `transientError` возвращается функцией `getTransactionAmountFromDB`.
```
Было getTransactionAmount : transientError -> Ошибка SQL
Стало getTransactionAmount : Ошибка -> transientError -> Ошибка SQL

В данном случае transientError: false
```
То, что возвращается в результате выполнения `getTransactionAmount`, не является непосредственно `transientError`: это обертка ошибки `transientError`.
...
func handler(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil {
        if errors.As(err, &transientError{}) { // Вызов errors.As с указателем на transientError в качестве аргумента
            http.Error(w, err.Error(), http.StatusServiceUnavailable) // Возврат 503, если ошибка является transient
        } else {
            http.Error(w, err.Error(), http.StatusBadRequest) // Возврат 400 в любом другом случае
        }
        return
    }
    // Текст ответа
