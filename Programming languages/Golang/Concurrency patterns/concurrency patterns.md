...
1. Когда у вас терабайты логов или веб-страниц, один сервер физически не может ни хранить, ни обработать такой объем за разумное время.
2. Написать код, который эффективно использует 1000 машин, учитывает сбои, пересылает данные между узлами и балансирует нагрузку — это огромная инженерная задача. Паттерн скрывает всю "грязную" работу по распределению, параллелизации, отказоустойчивости и синхронизации за простыми абстракциями Map и Reduce.
3. Даже на одном мощном сервере попытка загрузить многогигабайтный файл в память для сортировки или подсчета вызовет OOM (Out Of Memory).

**Суть**: Разделить задачу обработки большого набора данных на два этапа:
1. Map (Сопоставление): Входные данные разбиваются на независимые фрагменты. Каждый фрагмент обрабатывается отдельной "маппер" функцией, которая преобразует его в набор промежуточных пар "ключ-значение".
2. Все промежуточные значения группируются по ключам. Для каждого уникального ключа вызывается "редьюсер" функция, которая агрегирует список значений в итоговый результат (часто одно значение).

Между этими этапами происходит неявный Shuffle (Перемешивание) — фаза, где фреймворк сортирует и пересылает данные от мапперов к редьюсерам так, чтобы все значения одного ключа попали в один редьюсер

**Ключевая идея**: "Разделяй и властвуй" для данных: разбей вычисление на множество независимых задач для параллельной обработки (Map), а затем собери и агрегируй промежуточные результаты по ключевым признакам (Reduce)

```mermaid
graph TD
    subgraph "Входные данные (Input)"
        A1[Split 0] --> M1
        A2[Split 1] --> M2
        A3[Split N] --> M3
    end

    subgraph "MAP Фаза (Параллельная)"
        direction LR
        M1[Mapper 1] -->|key, value| Inter1[(Промежуточные данные<br/>на локальном диске)]
        M2[Mapper 2] -->|key, value| Inter2[(Промежуточные данные<br/>на локальном диске)]
        M3[Mapper N] -->|key, value| InterN[(Промежуточные данные<br/>на локальном диске)]
    end

    subgraph "SHUFFLE Фаза (Сортировка & Пересылка)"
        Inter1 --> Part{Разделение по ключу}
        Inter2 --> Part
        InterN --> Part
        Part -->|Ключи K1, K2| R1[Reducer 1]
        Part -->|Ключи K3, K4| R2[Reducer 2]
        Part -->|Ключи KM, KN| RN[Reducer N]
    end

    subgraph "REDUCE Фаза (Агрегация)"
        R1 --> Out1[Часть результата]
        R2 --> Out2[Часть результата]
        RN --> OutN[Часть результата]
    end

    Out1 & Out2 & OutN --> Final[Финальный агрегированный результат]

    style M1 fill:#lightblue,stroke:#333
    style M2 fill:#lightblue,stroke:#333
    style M3 fill:#lightblue,stroke:#333
    style Part fill:#lightgreen,stroke:#0f0,stroke-width:2px
    style R1 fill:#lightyellow,stroke:#333
    style R2 fill:#lightyellow,stroke:#333
    style RN fill:#lightyellow,stroke:#333
```

### Отличие MapReduce от других паттернов

1. **Отличие от Pipeline.**

* Pipeline: Данные проходят последовательные этапы трансформации на одной машине (или в одном процессе). Каждый этап получает данные из канала и отдает в следующий канал. Это про организацию кода внутри одного приложения.

* MapReduce: Операции над распределенным набором данных на кластере. Ключевая фаза Shuffle (перемешивание) подразумевает пересылку данных по сети между узлами, чего нет в классическом pipeline'е .

2. **Отличие от Fan-out / Fan-in.**

* Fan-out/Fan-in: Паттерн для параллелизации в рамках одного процесса/машины. Данные из одного канала раскидываются по воркерам и собираются обратно. Нет понятия "ключа" и глобальной сортировки.

* MapReduce: Похож на глобальный Fan-out/Fan-in, но масштабированный на кластер и обогащенный фазой Shuffle (сортировка и группировка по ключу), которая критична для правильной работы Reduce .

3. **Отличие от обычных функций Map и Reduce в программировании.**

* Встроенные функции (например, slices.Map): Просто применяют функцию к каждому элементу слайса и возвращают новый слайс. Выполняются последовательно.

* Паттерн MapReduce: Подразумевает распределенное выполнение, отказоустойчивость, работу с данными, не помещающимися в память, и обязательную фазу группировки по ключу.
